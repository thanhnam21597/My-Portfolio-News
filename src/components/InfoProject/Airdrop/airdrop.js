...
function Airdrop() {
  ...
  const loadPosts = async () => {
    try {
      ...
      const filtered = data.filter((post) => post.category === 'do-dien-tu');
      setPosts(filtered.reverse());
    } catch (error) {
      console.error('Error loading Airdrop posts:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <Container fluid className='project-section'>
      ...
      <Container>
        ...
        <h1 className='project-heading'>
          Airdrop <strong className='purple'>Info</strong>
        </h1>


        {posts.length === 0 ? (
          <p style={{ color: '#ddd' }}>
            Chưa có bài đăng Airdrop. Vui lòng tạo bài ở trang Đăng tin và chọn chuyên mục Airdrop.
          </p>
        ) : (
          <>
            <h4 className='text-light'>Bài đăng Airdrop của bạn</h4>
            <Row style={{ justifyContent: 'center', paddingBottom: '20px' }}>
              {posts.map((post) => (
                <Col md={6} key={post.id} className='mb-4'>
                  <div className='p-3' style={{ backgroundColor: '#1e1e2d', borderRadius: '10px' }}>
                    {post.image && (
                      <div style={{ marginBottom: '12px', textAlign: 'center', cursor: 'pointer' }} onClick={() => handleShowPost(post)}>
                        <img
                          src={post.image}
                          alt={post.title}
                    ...
                    <small style={{ color: '#aaa' }}>
                      Chuyên mục: Airdrop  {new Date(post.created_at).toLocaleString()}
                    </small>
                    {currentUser && (
                      <div className="mt-2">
                        <ButtonGroup>
          ...
          <Modal.Body style={{ backgroundColor: '#121223', color: '#eee' }}>
            {selectedPost && (
              <>
                ...
                <p style={{ color: '#bbb' }}>
                  <strong>Chuyên mục:</strong> Airdrop<br />
                  <strong>Ngày đăng:</strong> {new Date(selectedPost.created_at).toLocaleString()}
                </p>
              </>
            )}
          </Modal.Body>
